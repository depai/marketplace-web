...
import { CartTelioContext } from './cart.context';
...
const getCartProducts = () => {
  const products = getLocalState('cartTelio');
  return products ? products : [];
};

const getTotalPrice = () => {
  const price = getLocalState('totalPriceTelio');
...
export const CartProvider = (props: any) => {
  ...
  const setPrice = (coupon = getCoupon()) => {
    ...
    setLocalState('totalPriceTelio', price);
    setTotalPrice(price);
  };
  ...
  const add = (event, item: Product) => {
    ...
    if (products.length) {
      ...
      // if the cartTelio is empty
      products.push({ ...item, quantity: 1 });
    }

    cartAnimation(event);

    setLocalState('cartTelio', products);
  ...
  const clearCart = () => {
    ...
    // setPrice();
    setLocalState('totalPriceTelio', 0);
    setTotalPrice(0);
